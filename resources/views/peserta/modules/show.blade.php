    ...
    @if($module->pdf_file)
        <div class="table-card" style="padding:1.5rem;">
            <h3 style="font-size:0.95rem; font-weight:700; color:var(--text); margin-bottom:1rem; display:flex; align-items:center; justify-content:space-between; flex-wrap:wrap; gap:0.5rem;">
                ...
                <a href="{{ route('peserta.modules.pdf', $module) }}" target="_blank" class="btn btn-secondary btn-sm" style="font-size:0.75rem;">
                    Buka di Tab Baru
                </a>
            </h3>
            <div style="border:1px solid var(--border); border-radius:8px; overflow:hidden; background:#fafafa;">
                <iframe src="{{ route('peserta.modules.pdf', $module) }}" style="width:100%; height:600px; border:none; display:block;"></iframe>
            </div>
        </div>
    @endif
